...
public enum BotState {
    ...
    Birthday{
        ...
        @Override
        public void enter(BotContext context) {
            sendMessage(context, "Enter your birth day:");
            sendMessage(context, "Date format: Day/Month/Year");
        }

        @Override
        public void handleInput(BotContext context) {
            String date = context.getInput();
            try {
                Date birthday = new SimpleDateFormat("dd/MM/yyyy").parse(date);
                context.getUser().setBirthday(birthday);
                next = Approved;
