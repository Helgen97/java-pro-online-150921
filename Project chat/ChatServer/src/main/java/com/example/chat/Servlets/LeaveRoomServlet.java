...
public class LeaveRoomServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (user != null) {
            roomsList.findRoomByName(roomName).deleteUser(user);
            response.getOutputStream().write("[SERVER] Leave chat success.".getBytes(StandardCharsets.UTF_8));
        } else {
